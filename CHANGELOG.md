...
- Standalone executables for macOS (arm64, x64), Linux (x64, arm64) and Windows (x64) are attached to every GitHub release, so `add-sg-mcp` runs without Node.js. Each release carries `SHA256SUMS` and a build provenance attestation per archive.
